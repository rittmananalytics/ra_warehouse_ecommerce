...
with source as (
  {{ filter_segment_relation(relation=var('stg_zendesk_messaging_fivetran_conversations_table')) }}
),
renamed as (
  SELECT
    concat('{{ var('stg_zendesk_messaging_id-prefix') }}',id) as conversation_id,
    concat('{{ var('stg_zendesk_messaging_id-prefix') }}',user.id) AS conversation_user_id,
    concat('{{ var('stg_zendesk_messaging_id-prefix') }}',conversation_message.author.id) AS conversation_author_id,
